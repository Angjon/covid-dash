...
# Brazil map was left aside for now
...
def create_navbar():
    ...
    nav_items = dbc.Nav(
        [
            dbc.NavItem(dbc.NavLink("Coronavirus Dashboard by Jonas Angulski", href="https://github.com/Angjon")),
            dbc.NavItem(
                dbc.NavLink(
                    "",
                    href="/",
...
def create_main_page():
    ...
    brazil_table = create_table("brazil")
    ...
    brazil_tab = create_tab(brazil_table, "Brazil", "brazil")
...
@app.callback(
    ...
    [
        ...
        Input("brazil-table", "active_cell"),
        Input("table-tabs", "value"),
    ],
    [
        State("world-table", "derived_virtual_data"),
        State("usa-table", "derived_virtual_data"),
        State("brazil-table", "derived_virtual_data"),
    ],
)
def change_area_graphs(world_cell, usa_cell, brazil_cell, group, world_data, usa_data, brazil_data):
    """
    Creates the three graphs in the upper right hand side

    Triggered when a country/state in the data table is clicked or when
    the World/USA/Brazil States tab is selected.

    Passes in BOTH the world/usa and brazil data table active cells and their
    ...
    We must use the tab value ("world" or "usa" or "brazil") to determine which table was
    selected by the user and which is the actual active cell.

    Inputs
    ------
    world_cell/usa_cell/brazil_cell: Active cell for world/usa/brazil table. Dash passes this in
    ...
    group: String value of the current tab above the data table.
            Values are either "world" or "usa" or "brazil".

    world_data/usa_data/brazil_data: All data from the data table as a list of dictionaries
    ...
    if group == "usa":
        ...
        area, cell, data = "State", brazil_cell, brazil_data
