...
GROUPS = "world", "usa", "brazil"
KINDS = "deaths", "cases"


class PrepareData:
    """
    Downloads the data from John Hopkins repository (USA / World) and applies several
    ...
    def select_columns(self, df):
        ...
        has_two_dashes = cols.str.count("-") == 2
        filt = is_area | has_two_slashes | has_two_dashes
        return df.loc[:, filt]
    ...
    def update_areas(self, df):
        ...
        df = df.replace({first_col:REPLACE_AREA})
        filtro = ~df[first_col].isin(['US','Brazil'])
        df = df[filtro]
        return df
...
def combine_all_data(cm, dm):
    ...
    brazil_cases_d = cm.combined_daily['brazil_cases']
    world_deaths_d = dm.combined_daily['world_deaths']
    usa_deaths_d = dm.combined_daily['usa_deaths']
    brazil_deaths_d = dm.combined_daily['brazil_deaths']

    # Add USA to world (daily)
    world_cases_d = world_cases_d.assign(USA=usa_cases_d.sum(axis=1))
    world_cases_d = world_cases_d.assign(Brazil=brazil_cases_d.sum(axis=1))
    world_deaths_d = world_deaths_d.assign(USA=usa_deaths_d.sum(axis=1))
    world_deaths_d = world_deaths_d.assign(Brazil=brazil_deaths_d.sum(axis=1))
    ...
    brazil_cases_c = cm.combined_cumulative['brazil_cases']
    world_deaths_c = dm.combined_cumulative['world_deaths']
    usa_deaths_c = dm.combined_cumulative['usa_deaths']
    brazil_deaths_c = dm.combined_cumulative['brazil_deaths']

    # Add USA to world (cumulative)
    world_cases_c = world_cases_c.assign(USA=usa_cases_c.sum(axis=1))
    world_cases_c = world_cases_c.assign(Brazil=brazil_cases_c.sum(axis=1))
    world_deaths_c = world_deaths_c.assign(USA=usa_deaths_c.sum(axis=1))
    world_deaths_c = world_deaths_c.assign(Brazil=brazil_deaths_c.sum(axis=1))
    ...
    df_brazil = pd.concat((brazil_deaths_d.stack(), brazil_cases_d.stack(), 
                    brazil_deaths_c.stack(), brazil_cases_c.stack()), axis=1, 
                   keys=['Daily Deaths', 'Daily Cases', 'Deaths', 'Cases'])
    
    df_all = pd.concat((df_world, df_usa, df_brazil), keys=['world', 'usa', 'brazil'], 
